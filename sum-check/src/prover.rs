...
impl<F: PrimeField> ProverInterface<F> for Prover<F> {
	...
	fn sum_check_proof(&mut self) -> SumCheckProof<F> {
		...
		for i in 1..self.polynomial.number_of_variables {
			...
			let verifier_random_response =
				F::from_be_bytes_mod_order(&self.transcript.sample_challenge());
...
mod tests {
	...
	fn test_compute() {
		...
		assert_eq!(poly.round_0_poly.evaluations, vec![Fq::from(2), Fq::from(10)]);
	}
	...
	fn test_compute_round_zero_poly() {
		...
		let mut prover = Prover::new(poly);
		prover.compute_round_zero_poly();
		assert_eq!(prover.round_0_poly.evaluations, vec![Fq::from(2), Fq::from(10)]);
	}
