...
impl<F: PrimeField> ComposedSumCheckInterface<F> for ComposedSumCheck<F> {
...
mod tests {
	...
	fn test_composed_calculate_sum() {
		...
		let mut prover = ComposedSumCheck::new(poly);
	...
	fn test_sum_check_proof() {
		...
		let mut sumcheck = ComposedSumCheck::new(composed);
